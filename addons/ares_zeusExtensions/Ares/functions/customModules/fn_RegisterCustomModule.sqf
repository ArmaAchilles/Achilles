...
if (isNil "Ares_Custom_Categories") then
{
	Ares_Custom_Categories = [];
};
...
if (not _replacedExistingEntry) then
{
	...
	_category_list = [_ctrl,Ares_category_list,_categoryName,_moduleDisplayName,_moduleClassName,_index] call Ares_fnc_AppendToModuleTree;
