...
//  DESCRIPTION: this function force AI to execute an animation in loop
//
//	ARGUMENTS:
...
_unit setVariable ["Achilles_var_animations",_avaiable_anims];
_unit setVariable ["Achilles_var_noWeapon", _noWeapon];
...
if (_noWeapon) then
{
	...
	_unit setVariable ["Achilles_var_primWeapon",_primWeapon];
...
Achilles_fnc_ambientAnim_terminate =
{
	...
	_unit setUnitPos "UP";
	_unit setVariable ["Achilles_var_animations", nil];
	_unit setVariable ["Achilles_var_noWeapon", nil];
...
[_unit,_combatReady] spawn
{
	...
	_unit setVariable ["Achilles_EhAnimDone", _ehAnimDone];
	...
	_unit setVariable ["Achilles_EhKilled", _ehKilled];
