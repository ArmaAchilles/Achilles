...
switch (_mode) do
{
	case "LOADED":
	{
		...
		_tree_ctrl ctrlAddEventHandler ["TreeSelChanged", "([""SELECTION_CHANGED""] + _this) call Ares_fnc_RscDisplayAttributes_spawnAdvancedComposition;"];
		...
		[] spawn 
		{
			if (true) then 
			{
...

				{
						...
						_enableSimulation = _x getVariable ["enabledSimulation", false];
				...
				{					
					...
					{
						["ADD",controlNull,[],_dialogResult select 2,_composition_details,_dialogResult select 1] call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition;
					} else
					{
						["ADD",controlNull,[],_dialogResult select 2,_composition_details] call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition;
					};
				};
			};
			createDialog "Ares_composition_Dialog";
			["LOADED"] call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition;
		};
	};
	case "EDIT_BUTTON":
	{
		[] spawn
		{
			if (true) then
			{
				...
				[
					...
					"Ares_fnc_RscDisplayAttributes_editAdvancedComposition"
				] call Ares_fnc_ShowChooseDialog;
				
				if (count _dialogResult == 0) exitWith {};
				if (_dialogResult select 0 == 1) then
				{
					["ADD",controlNull,[],_dialogResult select 2,_item_details,_dialogResult select 1] call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition;
				} else
				{
					["ADD",controlNull,[],_dialogResult select 2,_item_details] call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition;
				};
				["REMOVE",controlNull,[],_category_name,_item_name] call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition;
			};
			waitUntil {!dialog};
			createDialog "Ares_composition_Dialog";
	...
	case "DELETE_BUTTON":
	{
		[] spawn
		{
			if (true) then
			{
				...
				(_dialog displayCtrl IDC_CONFIRM_WARNING) ctrlAddEventHandler ["ButtonClick","([""REMOVE"",controlNull,[]," + str _category_name + "," + str _item_name  + "]) call Ares_fnc_RscDisplayAttributes_manageAdvancedComposition; closeDialog 1;"];
