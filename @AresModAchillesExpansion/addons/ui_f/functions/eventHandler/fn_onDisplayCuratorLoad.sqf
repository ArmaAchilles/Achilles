...
#define ALL_CTRL_CREATE_IDCS	[IDC_RSCDISPLAYCURATOR_CREATE_UNITS_WEST, IDC_RSCDISPLAYCURATOR_CREATE_UNITS_EAST, IDC_RSCDISPLAYCURATOR_CREATE_UNITS_GUER, IDC_RSCDISPLAYCURATOR_CREATE_UNITS_CIV, IDC_RSCDISPLAYCURATOR_CREATE_UNITS_EMPTY, IDC_RSCDISPLAYCURATOR_CREATE_GROUPS_WEST, IDC_RSCDISPLAYCURATOR_CREATE_GROUPS_EAST, IDC_RSCDISPLAYCURATOR_CREATE_GROUPS_GUER, IDC_RSCDISPLAYCURATOR_CREATE_GROUPS_CIV, IDC_RSCDISPLAYCURATOR_CREATE_GROUPS_EMPTY]
...
_this spawn
{
	...
	// Fixes the loss of sides in the module tree caused by an ongoing search
	{
		...
		{
			private _display = ctrlParent param [0];
			private	_ctrlSearch = _display displayCtrl IDC_RSCDISPLAYCURATOR_CREATE_SEARCH;
			private _searchText = ctrlText _ctrlSearch;
			(missionNamespace getVariable ["RscDisplayCurator_sections", [0,0]]) params ["_curMode","_curSide"];
			if (_searchText != "") then
			{
				private _ctrlCreateList = ALL_CTRL_CREATE_IDCS apply {_display displayCtrl _x};
				if (_curMode <= 1) then
				{
					systemChat str [_curMode,_curSide];
