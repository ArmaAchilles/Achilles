...
{	// create a new dict
	...
	{
		_temp_ref = [_path_by_categories select _i];
...
} else
{
	...
	for "_i" from 0 to ((count _path_by_categories) - 1) do
	{
		...
		_category_list = _input_dict select _i;
		{
			_category_list = _category_list select _x;
		} forEach _path_by_index;
		
		// find the i'th category given by input
		_category = _nested_categories select _i;
		_category_index = _category_list find _category;
