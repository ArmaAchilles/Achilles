/*
	...
	Parameters:
		...
		_weaponsAndMuzzlesAndMagazines	- <ARRAY> An element of the array returned by Achilles_fnc_getWeaponsMuzzlesMagazines
...
[
	"_aircraft",
	"_target",
	"_weaponsAndMuzzlesAndMagazines",
	["_offset_custom", 0, [0]]
];
if (not (_aircraft isKindOf "Plane")) exitWith {false};
if (not alive _aircraft) exitWith {false};
...
if ((_vectDir_start#0 toFixed 1) in ["0.0","-0.0"] and ((_vectDir_start#1 toFixed 1) in ["0.0","-0.0"])) exitWith 
{
...
{
	["STR_AMAE_ARCRAFT_IS_TOUCHING_GROUND_ERROR"] call Achilles_fnc_showZeusErrorMessage;
	false
};

// Get the selected weapon
(_weaponsAndMuzzlesAndMagazines#0) params ["_weapon"];
...
switch (_weaponType) do
{
	...
	{
		_offset_weapon = _offset_weapon + 75;
		_maxRounds = MISSILES_PER_STRIKE;
	};
	default 
	{
		_offset_weapon = _offset_weapon + 320;
		_maxRounds = 1;
	};
};
...
if (_pos_target distance2D _pos_bankedTurn_start < (RADIUS_AO + _offset_weapon + 2*RADIUS_BANKED_TURN)) then
{
	...
	private _c = _pos_bankedTurn_start#0^2 - 2*_pos_bankedTurn_start#0*_pos_target#0 + _pos_bankedTurn_start#1^2 - 2*_pos_bankedTurn_start#1*_pos_target#1 + _pos_target#0^2 + _pos_target#1^2 - (RADIUS_AO + 2*RADIUS_BANKED_TURN)^2;
	comment "The displacement has to be in the direction of travel, hence we are only interested in the positive solution";
...
{
	params ["_t"];
	comment "_g is the traveled distance on the xy-plane";
	...
	comment "Bank angle based on linear interpolation";
...
{
	comment "Rotate in clockwise sense as it is shorter";
...
// Approach AO trajectory (just the params for setVelocityTransformation)
...
private _radius_cas_start = DIST_CAS_START * cos(PITCH_CAS);
private _distXY_preCAS = RADIUS_AO - _radius_cas_start;
// Duration and acceleration in xy-plane
...
if (!alive _aircraft || !canMove _aircraft || !alive driver _aircraft || fuel _aircraft == 0) exitWith {false};

// Start firing
private _fireHandle = [_aircraft, _weaponsAndMuzzlesAndMagazines, _maxRounds, _target] spawn
{
	params ["_aircraft", "_weaponsAndMuzzlesAndMagazines", "_maxRounds", "_target"];
	comment "Launch flares";
	[_aircraft, [5, 1.1]] call Achilles_fnc_LaunchCM;
	comment "Get weapon/mag details";
	(_weaponsAndMuzzlesAndMagazines#0) params ["_muzzle","_turretPath"];
	(_weaponsAndMuzzlesAndMagazines#1#0#1) params ["_magazine"];
	private _gunner = _aircraft turretUnit _turretPath;
	_aircraft selectWeaponTurret [_muzzle, _turretPath];
	private _mode = (weaponState [_aircraft, _turretPath, _muzzle])select 2;
	private _reloadTime = 0.1;
	if (_mode == "this") then
	{
		_reloadTime = getNumber (configFile >> "CfgWeapons" >> _muzzle >> "reloadTime");
	}
	else
	{
		_reloadTime = getNumber (configFile >> "CfgWeapons" >> _muzzle >> _mode >> "reloadTime");
	};
