...
if (_surrenderUnit) then
{
	...
	{
		case "tie": { 
			if (isLocalized "STR_AMAE_TIE_UNIT") then { localize "STR_AMAE_TIE_UNIT" } else { "Tie Unit" }
		};
		default {
			if (isLocalized "STR_AMAE_TIE_UNIT") then { localize "STR_AMAE_RELEASE_UNIT" } else { "Release Unit" }
		};
	};
