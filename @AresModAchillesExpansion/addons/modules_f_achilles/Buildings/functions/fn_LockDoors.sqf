...

{
	...
	_source_cfg = [(configFile >> "cfgVehicles" >> typeOf _building  >> "AnimationSources"), 0] call BIS_fnc_subClasses;
	{
		...
		if (toLower (_content select 2) == "sound") then
		{
			...
			_trigger_pos_list pushBack _trigger_pos;
...
// remove old logics
{
	_close_logics = _x nearObjects ["Module_f", 1];
	if(count _close_logics > 0) then {deleteVehicle (_close_logics select 0)};
} forEach _trigger_pos_list;

if (_mode < 2) then 
{
	// delayed post modifications are curucial!
	uiSleep 5;
	if (count _logic_list == 0) exitWith {deleteGroup _logic_group};
	_sourceObject_list = [];
