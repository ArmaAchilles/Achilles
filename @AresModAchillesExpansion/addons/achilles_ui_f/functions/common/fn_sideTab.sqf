...
_index = _this param [2,0,[0]];
_default_side_index = _this param [3,1,[0]];
...
switch _mode do 
{
	case "onLoad": 
	{
		...
		// Prepare Side Icons
		{
			...
			_color = if (_foreachindex == 0) then {[1,1,1,1]} else {(_foreachindex - 1) call bis_fnc_sideColor};
	...
	case "selectSide":
	{
		...
		if (_old_side_index != -1) then
		{
			...
			_old_side_ctrl = _display displayCtrl _old_side_idc;
