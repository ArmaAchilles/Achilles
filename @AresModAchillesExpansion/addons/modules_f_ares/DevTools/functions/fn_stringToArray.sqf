...
for "_idx" from (count _charArray -1) to 0 step -1 do
{
	private _char = _charArray select _idx;
	switch (true) do
	{
		// remove whitespace outside strings
		case (_char in [0x09, 0x0A, 0x0D,0x20] && {!_isInsideQuote}): 
		{
			_charArray deleteAt _idx;
		};
		// handle apostroph
		...
		case (_char isEqualTo 0x22):
		{
			if (_isApostrophQuote) then
			{
				...
				_charArray = (_charArray select [0, _idx + 1]) + (_charArray select [_idx, count _charArray - _idx]);
			}
			else
			{
				// enter/exit quote
